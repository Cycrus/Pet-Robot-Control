 ...
 * URL: https://gi.thub.com/gavinlyonsrepo/ST7735_TFT_RPI
 * 
...
void ST7735_TFT_graphics ::TFTdrawPixel(uint8_t x, uint8_t y, uint16_t color) {
	uint16_t pixel_id = ST7735_Utils::coords2DTo1D(x, y + 1, _widthTFT);
	
	if(pixel_colors_.at(pixel_id) != color)
	{
		...
		pixel_colors_.at(pixel_id) = color;
	}
}
...
void ST7735_TFT_graphics ::TFTfillScreen(uint16_t color) {
	...
	std::fill(std::begin(pixel_colors_), std::begin(pixel_colors_) + pixel_num_,
              color);
}
...
void ST7735_TFT_graphics ::TFTdrawBitmap(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color, uint16_t bgcolor, const unsigned char bitmap[]) {
...
void ST7735_TFT_graphics ::TFTdrawBitmap16(uint8_t x, uint8_t y, uint8_t *pBmp, char w, char h) {
	...
	for(j = 0; j < h; j++)
	{
		for(i = 0; i < w; i ++)
		{
			//Note:This code(next two lines) works too, left in for reference
...
// Desc: Draws a 16 bit color canvas to the screen
// Param 1,2  X,Y screen co-ord
// Param 3 A pointer to the databuffer containing the canvas data
// Param 4,5: width and height 0-127 possible values width and height of bitmap in pixels

void ST7735_TFT_graphics ::TFTdrawCanvas16(uint8_t x, uint8_t y, uint16_t *pBmp, char w, char h) {
	uint8_t i, j;
	uint16_t color;
