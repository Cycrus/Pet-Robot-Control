...
// Desc: Returns the current pixel colors.
std::vector<uint16_t> ST7735_TFT::getPixelColors()
{
	return pixel_colors_;
}

uint8_t ST7735_TFT::getWidth()
{
	return _widthTFT;
}

uint8_t ST7735_TFT::getHeight()
{
	return _heightTFT;
}
...
void ST7735_TFT  :: TFTInitScreenSize(uint8_t colOffset, uint8_t rowOffset, uint16_t width_TFT, uint16_t height_TFT)
{
	...
	pixel_num_ = width_TFT * height_TFT;
	pixel_colors_.insert(std::begin(pixel_colors_), pixel_num_, ST7735_WHITE);
